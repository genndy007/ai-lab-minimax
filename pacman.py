...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    if (numGames-numTraining) > 0:
        ...
        win = wins[0]
        final_score = int(scores[0])
        algo = pacman.__class__.__name__
        write_to_csv(win, final_score, algo)
...
def write_to_csv(win, score, algo):
    name = 'results.csv'
    f = open(name, 'a')
    f.write(str(win)+','+str(score)+','+str(algo)+'\n')
    f.close()
